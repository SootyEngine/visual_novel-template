...
func _redraw():
	var text := []
	var meta := {}
	for mod in Sooty.mods.get_installed() + Sooty.mods.get_uninstalled():
		var btn := ""
		if mod.installed:
			btn = "[meta %s;tomato]Uninstall[]" % [mod.dir]
			meta[mod.dir] = Sooty.mods.uninstall.bind(mod.dir)
		else:
			btn = "[meta %s;yellow_green]Install[]" % [mod.dir]
			meta[mod.dir] = Sooty.mods.install.bind(mod.dir)
