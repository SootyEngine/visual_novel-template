...
func _redraw():
	var text := []
	var meta := {}
	for mod in ModManager.get_installed() + ModManager.get_uninstalled():
		var btn := ""
		if mod.installed:
			btn = "[meta %s;tomato]Uninstall[]" % [mod.dir]
			meta[mod.dir] = ModManager.uninstall.bind(mod.dir)
		else:
			btn = "[meta %s;yellow_green]Install[]" % [mod.dir]
			meta[mod.dir] = ModManager.install.bind(mod.dir)
